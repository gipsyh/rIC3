...
def write_shadow_outputs(
    ...
    text = [
        ...
        "// The modules below preserve elaborated hierarchy and names for CIll invariants.",
        "",
    ]
...
def parse_args(argv=None):
    parser = argparse.ArgumentParser(
        description="Generate CIll shadow DUT artifacts for ric3 cill prepare."
    )
...
def main(argv=None) -> int:
    ...
    shadow, link_map_path = generate_shadow(
        args.top, [path.resolve() for path in args.file], {}, out_dir
    )
    print("\nGenerated shadow DUT artifacts:", flush=True)
    for path in [shadow, link_map_path]:
